    ...
    public static class AgeFunction
    {
        ...
        public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = null)]HttpRequestMessage req, TraceWriter log)
        {
            try
            {
                ...
                log.Info($"AgeFunction - Url: {url}");
                log.Info($"AgeFunction - ApiKey: {apiKey}");
                log.Info($"AgeFunction - Endpoint: {domainEndpoint}");
                ...
                var result = await service.DetectAsync(HttpUtility.UrlDecode(url), true, false, faceAttributes);
                var emotionResult = result?.Select(r => new { r.FaceId, r.FaceRectangle, r.FaceAttributes.Age });
